...
class TestRunner(object):
    ...
    def do_CRNL(self, term):
        self.flags["crnl"] = bool(self.getInt())

    def do_REP(self, term):
        count = self.getInt()
        for i in range(count):
            TestRunner(self.text, self.flags).doCommand(term)
    ...
    def do_SEQ(self, term):
        ...
        for i in range(lo, hi + 1):
            lflags["seq"] = i
            TestRunner(self.text, lflags).doCommand(term)
